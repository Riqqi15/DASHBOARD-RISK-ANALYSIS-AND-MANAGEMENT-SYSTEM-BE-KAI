  ...
  <article class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
    <div class="border-l-4 border-[#2d2a70] p-4">
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
          <p class="font-mono text-sm font-semibold tracking-wide text-[#2d2a70]">{{ row.spare_part.code }}</p>
          <h3 class="mt-1 text-sm font-semibold leading-5 text-slate-950">{{ row.spare_part.detail_equipment }}</h3>
          <p v-if="row.spare_part.equipment" class="mt-1 text-sm text-slate-500">{{ row.spare_part.equipment }}</p>
        </div>
        <span class="shrink-0 rounded-full border px-2.5 py-1 text-sm font-semibold" :class="meta(row.status).class">{{ meta(row.status).label }}</span>
      </div>
      ...
      <div class="mt-4 grid grid-cols-2 gap-px overflow-hidden rounded-lg border border-slate-200 bg-slate-200">
        <div class="bg-slate-50 px-3 py-2.5">
          <p class="text-sm font-semibold uppercase tracking-wider text-slate-500">Stok tercatat</p>
      ...
      <button type="button" class="mt-4 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-[#171650] outline-none hover:bg-slate-50 focus:ring-2 focus:ring-[#2d2a70] focus:ring-offset-2" :aria-label="`Catat IN/OUT ${row.spare_part.detail_equipment}`" @click="$emit('movement', row)">
        <ArrowDownToLine :size="16" aria-hidden="true" /><ArrowUpFromLine :size="16" aria-hidden="true" /> Catat IN/OUT
      </button>
    </div>
  </article>
</template>
