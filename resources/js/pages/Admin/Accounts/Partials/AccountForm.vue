...
<template>
  <form class="space-y-5" @submit.prevent="submit">
    <div class="grid gap-5 md:grid-cols-2">
      ...
      <div><label for="email" class="mb-2 block text-sm font-medium">Email</label><input id="email" v-model="form.email" type="email" :class="input" required autocomplete="username"><p v-if="form.errors.email" class="mt-2 text-sm text-red-600" role="alert">{{ form.errors.email }}</p></div>
    </div>
