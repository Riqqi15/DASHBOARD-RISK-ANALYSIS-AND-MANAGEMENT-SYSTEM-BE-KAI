    ...
    <div v-if="dialogOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm" @click.self="closeDialog">
      <section class="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-2xl">
        ...
        <form class="grid gap-4 p-6 md:grid-cols-2" @submit.prevent="submit">
          <div class="md:col-span-2"><label for="risk-register-asset" class="mb-1.5 block text-sm font-semibold text-slate-700">Aset</label><select id="risk-register-asset" v-model="form.asset_id" required class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm"><option value="">Pilih aset</option><option v-for="asset in assets" :key="asset.id" :value="asset.id">{{ asset.unit?.code }} · {{ asset.name }}</option></select><span v-if="form.errors.asset_id" class="mt-1 block text-xs text-red-600">{{ form.errors.asset_id }}</span></div>
          <div><label for="risk-register-part-number" class="mb-1.5 block text-sm font-semibold text-slate-700">Part Number</label><input id="risk-register-part-number" v-model="form.part_number" class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm" /></div>
          <div><label for="risk-register-sub" class="mb-1.5 block text-sm font-semibold text-slate-700">Subsystem/Sub</label><input id="risk-register-sub" v-model="form.sub" class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm" /></div>
          <div class="md:col-span-2"><label for="risk-register-event" class="mb-1.5 block text-sm font-semibold text-slate-700">Peristiwa Risiko</label><input id="risk-register-event" v-model="form.risk_event" required class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm" /><span v-if="form.errors.risk_event" class="mt-1 block text-xs text-red-600">{{ form.errors.risk_event }}</span></div>
          <div class="md:col-span-2"><label for="risk-register-cause" class="mb-1.5 block text-sm font-semibold text-slate-700">Penyebab</label><textarea id="risk-register-cause" v-model="form.risk_cause" required rows="3" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" /></div>
          <div><label for="risk-register-likelihood" class="mb-1.5 block text-sm font-semibold text-slate-700">Likelihood (1–4)</label><input id="risk-register-likelihood" v-model.number="form.likelihood" type="number" min="1" max="4" required class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm" /></div>
          <div><label for="risk-register-consequence" class="mb-1.5 block text-sm font-semibold text-slate-700">Consequence (1–4)</label><input id="risk-register-consequence" v-model.number="form.consequence" type="number" min="1" max="4" required class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm" /></div>
          <div class="md:col-span-2"><label for="risk-register-impact" class="mb-1.5 block text-sm font-semibold text-slate-700">Dampak</label><textarea id="risk-register-impact" v-model="form.impact" rows="2" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" /></div>
          <div><label for="risk-register-part-name" class="mb-1.5 block text-sm font-semibold text-slate-700">Nama Part</label><input id="risk-register-part-name" v-model="form.part_name" class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm" /></div>
          <div><label for="risk-register-form-status" class="mb-1.5 block text-sm font-semibold text-slate-700">Status</label><select id="risk-register-form-status" v-model="form.status" class="h-10 w-full rounded-lg border border-slate-300 px-3 text-sm"><option value="open">Open</option><option value="in_progress">In Progress</option><option value="closed">Closed</option></select></div>
          <div class="md:col-span-2"><label for="risk-register-recommendation" class="mb-1.5 block text-sm font-semibold text-slate-700">Rekomendasi</label><textarea id="risk-register-recommendation" v-model="form.recommendation" rows="3" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm" /></div>
