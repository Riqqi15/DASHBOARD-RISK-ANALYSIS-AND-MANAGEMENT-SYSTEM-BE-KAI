...
final class RiskRegisterManagementTest extends TestCase
{
    ...
    public function test_unit_user_only_sees_risks_and_assets_from_its_unit(): void
    {
        ...
        $asset = Asset::factory()->for($unit)->create(['nama_aset' => 'Aset Sendiri']);
        $otherAsset = Asset::factory()->for($other)->create(['nama_aset' => 'Aset Lain']);
    ...
    public function test_unit_user_can_create_update_and_delete_risk_for_its_asset(): void
    {
        ...
        $this->actingAs($user)->post('/risk-register', $this->payload($asset, [
        ...
        $this->actingAs($user)->put("/risk-register/{$risk->id}", $this->payload($asset, [
            'status' => 'in_progress',
            'recommendation' => 'Ganti relay pada inspeksi berikutnya',
        ]))->assertRedirect();

        $this->assertSame('in_progress', $risk->refresh()->status->value);
        $this->actingAs($user)->delete("/risk-register/{$risk->id}")->assertRedirect();
    ...
    public function test_unit_user_cannot_mutate_risk_from_another_unit(): void
    {
        ...
        $this->actingAs($user)->put("/risk-register/{$otherRisk->id}", $this->payload($ownAsset))->assertForbidden();
