...
class InventoryIndexTest extends TestCase
{
    ...
    public function test_regional_index_is_authorized_scoped_and_keeps_inactive_historical_names(): void
    {
        ...
        $this->actingAs($user)
            ->get('/inventory?unit_kerja_id='.$otherUnit->id)
            ->assertOk()
            ->assertInertia(
                fn (Assert $page) => $page
    ...
    public function test_pusat_selected_unit_and_default_both_scope_all_operational_inventory_data(): void
    {
        ...
        StockMovement::factory()
            ...
            ->for($part)
            ->for($pusat, 'actor')
            ->create(['movement_date' => '2026-07-01']);
        StockMovement::factory()
            ->for($secondUnit)
            ->for($part)
            ->for($pusat, 'actor')
            ->create(['movement_date' => '2026-07-02']);

        $this->actingAs($pusat)
            ->get('/inventory?unit_kerja_id='.$firstUnit->id)
            ->assertInertia(
                fn (Assert $page) => $page
                    ->has('stocks.data', 1)
    ...
    public function test_history_marks_only_uncorrected_original_movements_as_correctable(): void
    {
        ...
        $inactivePartMovement = StockMovement::factory()
            ->for($activeUnit)
            ->for($inactivePart)
            ->for($user, 'actor')
            ->create();
        ...
        $deletedPartMovement = StockMovement::factory()
            ->for($activeUnit)
            ->for($deletedPart)
            ->for($user, 'actor')
            ->create();
        ...
        $inactiveUnitMovement = StockMovement::factory()
            ->for($inactiveUnit)
            ->for($activePart)
            ->for($user, 'actor')
            ->create();
        ...
        $deletedUnitMovement = StockMovement::factory()
            ->for($deletedUnit)
            ->for($activePart)
            ->for($user, 'actor')
            ->create();
        ...
        $this->actingAs($user)->get('/inventory?tab=history')->assertInertia(
            fn (Assert $page) => $page->where('movements.data', function ($rows) use (
                $corrected,
                $available,
                $correction,
    ...
    public function test_stock_status_boundaries_drive_rows_filters_and_below_reorder_stat(): void
    {
        ...
        $this->actingAs($user)->get('/inventory')->assertInertia(
            fn (Assert $page) => $page
                ->where('stocks.data', function ($rows) use ($cases): bool {
                    $statuses = collect($rows)->mapWithKeys(
                        fn (array $row): array => [
                            $row['spare_part']['code'] => $row['status'],
                        ],
                    );

                    return collect($cases)->every(fn (array $case): bool => $statuses->get($case[0]) === $case[4]);
                })
        ...
        foreach (['empty' => 1, 'critical' => 2, 'below_reorder' => 3, 'available' => 2] as $status => $count) {
            $this->actingAs($user)
                ->get('/inventory?stock_status='.$status)
                ->assertInertia(
                    fn (Assert $page) => $page
    ...
    public function test_search_and_category_filters_match_part_and_hierarchy_names(): void
    {
        ...
        $radio = SparePart::factory()
            ->for($telecomSubsystem)
            ->create(['detail_equipment' => 'Antenna Radio']);
        ...
        $this->actingAs($user)
            ->get('/inventory?search=Daop+Jakarta')
    ...
    public function test_invalid_filters_are_normalized_without_errors(): void
    {
        ...
        $this->actingAs($pusat)
            ...
            ->assertOk()
            ->assertInertia(
                fn (Assert $page) => $page->where('filters', [
                    'search' => '',
                    'asset_group_id' => '',
                    'asset_subsystem_id' => '',
                    'stock_status' => 'all',
    ...
    public function test_master_client_page_is_normalized_and_round_trips_without_changing_inventory_scope(): void
    {
        ...
        $this->actingAs($user)
            ->get('/inventory?tab=master&master_page=2')
            ->assertOk()
            ->assertInertia(
                fn (Assert $page) => $page
    ...
    public function test_stock_and_movement_pagination_are_independent_and_preserve_filters(): void
    {
        ...
        $this->actingAs($user)
            ->get('/inventory?search=matched&page=2&movement_page=2')
            ->assertInertia(
                fn (Assert $page) => $page
    ...
    public function test_history_filters_by_type_date_and_selected_unit(): void
    {
        ...
        $pusat = User::factory()->pusat()->create();
        $part = SparePart::factory()->create();
        $matching = StockMovement::factory()
            ->for($unit)
            ->for($part)
            ->for($pusat, 'actor')
            ->create([
    ...
    public function test_history_date_filters_use_sargable_date_column_comparisons(): void
    {
        ...
        $movementSql = $queries->filter(fn (string $sql): bool => str_contains($sql, 'stock_movements'))->implode(' ');
    ...
    public function test_access_policy_reorder_redirect_and_props_contain_no_dummy_metrics(): void
    {
        ...
        $this->actingAs($user)->get('/reorder-stock')->assertRedirect('/inventory?tab=master');
        $this->actingAs($user)
            ->get('/inventory')
            ->assertInertia(
                fn (Assert $page) => $page
    ...
    public function test_master_asset_hierarchy_adds_ledger_to_baseline_without_cross_unit_leakage(): void
    {
        ...
        $pusat = User::factory()->pusat()->create();
        $this->actingAs($pusat)
            ->get('/master-asset?unit_kerja_id='.$ownUnit->id)
            ->assertInertia(
                fn (Assert $page) => $page
                    ->where('hierarchy.0.sparepart_in', 13)
                    ->where('hierarchy.0.sparepart_out', 6),
    ...
    public function test_invalid_pusat_hierarchy_unit_selection_falls_back_without_aggregate_leakage(): void
    {
        ...
        foreach ([$inactiveUnit->id, 999_999] as $invalidUnitId) {
            $this->assertTrue(
                app(AssetHierarchyQuery::class)
                    ->forUser($pusat, $invalidUnitId, [$subsystem->id])
                    ->isEmpty(),
            );

            $this->actingAs($pusat)
                ->get('/master-asset?unit_kerja_id='.$invalidUnitId)
                ->assertOk()
                ->assertInertia(
                    fn (Assert $page) => $page
                        ->where('filters.unit_kerja_id', (string) $firstUnit->id)
