    ...
    public function up(): void
    {
        if (! Schema::hasColumn('spare_parts', 'reorder_calculation_status')) {
            Schema::table('spare_parts', function (Blueprint $table): void {
                ...
                $table->string('reorder_formula_version', 30)->default('kai-reorder-v1.0.0')->after('reorder_calculation_status');
        ...
        DB::table('spare_parts')
            ...
            ->update([
                'safety_stock' => DB::raw('CEIL(GREATEST(0, (max_yearly_failure * max_lead_time_months) - (average_yearly_failure * average_lead_time_months)))'),
                'lead_time_demand' => DB::raw('CEIL(average_yearly_failure * average_lead_time_months)'),
                'reorder_point' => DB::raw('CEIL(GREATEST(0, (max_yearly_failure * max_lead_time_months) - (average_yearly_failure * average_lead_time_months)) + (average_yearly_failure * average_lead_time_months))'),
        ...
        DB::table('spare_parts')
            ->where(function ($query): void {
                $query->whereNull('max_yearly_failure')
