...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        DB::table('failure_logs')->whereIn('source_key', $this->legacyFailureSourceKeys())->delete();
    ...
    {
        return array_map(
            fn (array $row): string => hash(
                'sha256',
                implode(
                    '|',
                    array_map(fn (string $part): string => mb_strtolower(trim($part)), [
                        'failure-log',
                        $row['unit'],
                        $row['subsystem'],
                        $row['started_at'],
